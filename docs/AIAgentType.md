...
| 设计内容 | 需要回答的问题 | 示例 |
|---|---|---|
| **用户对象** | 谁使用 Agent | 1. 电力交易员：制定日前、日内及辅助服务市场交易策略。2. 调度运行人员：安排机组、储能及可调负荷运行计划。3. 综合能源运营人员：协调电、热、冷、气等多能系统运行。4. 风险管理人员：评估价格、负荷、发电及交易履约风险。5. 合规与审计人员：审核交易方案是否符合市场规则、监管要求及企业制度。6. 管理人员：查看经营指标、收益预测和重大风险提示。 |
| **业务任务** | Agent 具体完成什么 | 由多个 Agent 协同完成交易或调度建议：1. **市场预测 Agent**：预测电价、市场供需、节点价格、辅助服务价格及市场波动趋势。2. **负荷预测 Agent**：预测园区、用户、区域或企业未来各时段的电、热、冷、气负荷。3. **发电预测 Agent**：预测风电、光伏、水电及常规机组的可用出力和不确定性。4. **储能优化 Agent**：根据价格、负荷、发电、SOC、功率和容量约束，制定充放电计划。5. **风险评估 Agent**：评估价格风险、偏差考核风险、弃风弃光风险、设备故障风险、现金流风险及履约风险。6. **合规审核 Agent**：检查交易时段、申报边界、持仓限额、报价规则、合同约束和操作权限。7. **协同决策模块**：综合预测结果、优化结果和风险约束，生成日前交易、日内调整、发电计划或储能调度建议，并提供多种备选方案。 |
| **输入信息** | 用户提供哪些数据 | 1. **市场数据**：历史及实时电价、日前/日内市场价格、节点或分区价格、交易规则、辅助服务价格、市场出清结果。2. **负荷数据**：历史负荷曲线、实时负荷、用户生产计划、天气、节假日、温度、湿度及特殊事件。3. **发电数据**：风光预测、机组装机容量、可用容量、爬坡速率、最小开停机时间、燃料成本、检修计划。4. **储能数据**：额定容量、最大充放电功率、当前 SOC、充放电效率、寿命衰减成本、运行上下限及可用状态。5. **合同与交易数据**：中长期合同、电量电价、偏差结算规则、已成交电量、持仓、交易限额和结算周期。6. **运营约束**：设备状态、网络容量、供热/供冷约束、碳排放约束、备用要求、系统安全边界。7. **企业规则**：收益目标、风险偏好、止损线、预算、审批权限和人工干预要求。 |
| **输出结果** | 用户最终得到什么 | 1. 分时段、分市场的交易建议，包括申报电量、报价区间、买卖方向、交易时机及预期收益。2. 发电、储能和可调负荷的运行计划，包括机组出力、储能充放电功率、SOC轨迹及负荷响应安排。3. 多场景方案，例如保守方案、基准方案和高收益方案，并展示收益、成本和风险差异。4. 预测结果及置信区间，包括价格、负荷、风光出力和偏差概率。5. 风险报告，包括风险等级、触发原因、潜在损失、敏感因素及应对措施。6. 合规审核结果，包括“通过、需人工复核、禁止执行”等状态，以及对应规则依据。7. 可解释的决策依据、关键假设、数据来源、模型版本和审计日志。8. 经授权后可生成待审批的交易单、调度指令或执行任务，但不默认直接提交市场或控制现场设备。 |
| **成功标准** | 怎样评价效果 | 1. **预测准确性**：价格、负荷、发电预测的 MAE、MAPE、RMSE、区间覆盖率等达到设定目标。2. **经济效益**：交易收益提升、购电成本下降、储能套利收益提高、燃料成本和偏差考核费用降低。3. **计划可执行性**：生成方案满足机组、储能、网络、合同、SOC、爬坡和安全约束，执行成功率高。4. **风险控制效果**：最大回撤、VaR、偏差风险、违约风险和异常损失不超过预设阈值。5. **合规性**：交易和调度建议通过规则校验，不发生因系统建议导致的违规申报、越权操作或超限运行。6. **稳定性与时效性**：在规定时间内完成预测、优化和审核，系统可用性高，异常情况下能够降级运行。7. **可解释性**：用户能够查看关键影响因素、方案对比、风险来源及推荐理由。8. **人工认可度**：交易员和调度员采纳率较高，人工修改原因可追踪，并能通过反馈持续改进。 |
| **风险边界** | 哪些事情不能自动完成 | 1. 不得在未经授权和人工审批的情况下，直接提交交易、签署合同、改变持仓或执行高风险市场操作。2. 不得绕过交易规则、监管要求、企业审批流程、权限管理和安全校验。3. 不得在数据缺失、数据异常、预测置信度过低或模型失效时，自动生成并执行确定性指令；应提示人工复核或切换保守策略。4. 不得突破机组、储能、电网及综合能源系统的物理约束，例如超容量充放电、SOC 越界、违反爬坡限制或影响供能安全。5. 不得以追求收益为由牺牲系统安全、供能可靠性、设备寿命或满足民生需求。6. 不得擅自修改原始数据、预测模型、市场规则和审计记录；关键结果应保留可追溯日志。7. 不得替代交易员、调度员、风险负责人和合规人员承担最终决策责任。8. 遇到极端天气、重大设备故障、市场异常波动、网络攻击或规则变化时，应暂停自动执行，升级人工处置。9. 涉及重大金额、重大安全影响、跨区域交易或异常报价的方案，必须经过分级审批和双人复核。 |

**示例协同流程：**  
市场预测 Agent 预测未来电价，负荷预测 Agent 和发电预测 Agent 提供供需边界，储能优化 Agent 生成充放电及交易组合方案，风险评估 Agent 计算收益和风险，合规审核 Agent 检查规则与权限，最终由交易员或调度员审核确认后执行。
