...
- **React 前端** (`web/`)：现代学习界面（答题、报告、AI 答疑）
- **FastAPI 后端** (`server.py`)：REST API 服务
- **DeepSeek LLM** (`edu_agent/llm_assistant.py`)：智能判分、解释生成、学习建议、AI 对话

### 配置

在项目根目录的 `config.env` 中填写你的 DeepSeek API Key：
...
# 2) 启动 FastAPI 后端 (http://localhost:8000)
uvicorn server:app --reload --port 8000

# 3) 安装并启动 React 前端 (http://localhost:5173)
cd web
npm install
npm run dev
```
...
| 方法 | 路径                      | 说明                          |
| ---- | ------------------------- | ----------------------------- |
| POST | `/api/session`            | 开始/恢复学习会话             |
| GET  | `/api/question/{user_id}` | 自适应获取下一道题            |
| POST | `/api/answer`             | 提交答案（DeepSeek 智能判分） |
| GET  | `/api/report/{user_id}`   | 学习报告                      |
| GET  | `/api/advice/{user_id}`   | DeepSeek 生成学习建议         |
| POST | `/api/chat`               | 与 DeepSeek AI 对话           |
