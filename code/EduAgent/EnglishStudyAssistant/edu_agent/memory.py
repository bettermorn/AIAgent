...
class MemoryDB:
    ...
    注意：FastAPI 同步接口运行在线程池中，多用户并发访问时
    读-改-写操作必须持锁进行，否则会互相覆盖导致数据丢失。
    """
    ...
    def rename_student(self, user_id: str, name: str) -> StudentProfile:
        """原子化更新学生姓名（读-改-写全程持锁，避免并发覆盖答题记录）"""
        with self._lock:
            data = self._read()
            students = data.setdefault("students", {})
    ...
    def log_interaction(self, profile: StudentProfile, record: QARecord) -> StudentProfile:
        """记录学习交互并更新技能统计
        
        注意：调用方传入的 profile 可能是数秒前读取的旧档案（如 LLM 判分期间），
        因此必须在锁内重新读取磁盘上的最新档案再追加，否则并发写入会互相覆盖。
        返回值为最新的学生档案。
        """
        with self._lock:
            data = self._read()
            students = data.setdefault("students", {})
            raw = students.get(profile.user_id)
            target = StudentProfile(**raw) if raw else profile
            
            target.history.append(record)
            
            # 更新每个技能标签的统计信息
            for tag in record.tags:
                stat = target.skills.get(tag, SkillStat())
                stat.update(record.is_correct, record.difficulty)
                target.skills[tag] = stat
                
            # 重新评估学生的CEFR等级
            target.update_level()
            self.save_student(target)
            return target
